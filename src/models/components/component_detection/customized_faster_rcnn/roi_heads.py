...
def maskrcnn_inference(x, labels, excluded_labels=None):
    # type: (Tensor, List[Tensor], Optional[List[int]]) -> List[Tensor]
    """
    ...
    Args:
        ...
        excluded_labels (Optional[List[int]]): if it is provided, the masks
            of for this labels will be None
    ...
    if excluded_labels is not None:
        ...
        for excluded_label in excluded_labels:
            label_idxs.append(torch.where(labels == excluded_label)[0])
        label_idxs = torch.cat(label_idxs)
        mask_prob = mask_prob[index, labels][:, None]
        excluded_indices = index[label_idxs]
        mask_prob[excluded_indices] = torch.nan
    else:
        mask_prob = mask_prob[index, labels][:, None]
...
def maskrcnn_loss(mask_logits,
                  ...
                  gt_labels,
                  mask_matched_idxs,
                  included_gt_labels=None):
    # type: (Tensor, List[Tensor], List[Tensor], List[Tensor], List[Tensor], Optional[List[int]]) -> Tensor
    """
    Args:
        mask_logits (Tensor)
    ...
    if included_gt_labels is not None:
        label_idxs = []
        for included_gt_label in included_gt_labels:
            label_idxs.append(torch.where(labels == included_gt_label)[0])
        label_idxs = torch.cat(label_idxs)
        logit_idxs = torch.arange(labels.shape[0], device=labels.device)[label_idxs]
        labels = labels[label_idxs]
        mask_targets = mask_targets[label_idxs]
        mask_loss = F.binary_cross_entropy_with_logits(
            # bu kullanım çok iyi, sadece gt_label in olduğu slice'i alıyor
            mask_logits[logit_idxs, labels], mask_targets
        )
    else:
        mask_loss = F.binary_cross_entropy_with_logits(
            # bu kullanım çok iyi, sadece gt_label in olduğu slice'i alıyor
...
class RoIHeads(nn.Module):
    ...
    def __init__(self,
        ...
        self.object_relation_module_fc1 = object_relation_module_fc1
        self.object_relation_module_fc2 = object_relation_module_fc2
    ...
    def postprocess_detections(self,
                               ...
                               relational_labels,  # type: List[int]
                               ):
        # type: (...) -> Tuple[List[Tensor], List[Tensor], List[Tensor], List[Dict]]
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list,
            ...
            # handling relational labels
            # we can enable score thresholding for relational boxes...
            # [this can be additional feature but i don't think it is needed] remove low scoring boxes
            # score_threshold_for_relations = 0.4
            # inds = torch.where(scores > score_threshold_for_relations)[0]
            # relation_boxes, relation_scores, relation_labels = boxes[inds], scores[inds], labels[
            #     inds]
            # rel_label_idxs = []
            # for relational_label in relational_labels:
            #     rel_label_idxs.append(torch.where(relation_labels == relational_label)[0])
            # rel_label_idxs = torch.cat(rel_label_idxs)
            # all_relational_data.append({
            #     'boxes': relation_boxes[rel_label_idxs],
            #     'labels': relation_labels[rel_label_idxs],
            # })

            rel_label_idxs = []
            for relational_label in relational_labels:
                rel_label_idxs.append(torch.where(labels == relational_label)[0])
            rel_label_idxs = torch.cat(rel_label_idxs)
            all_relational_data.append({
                'boxes': boxes[rel_label_idxs],
                'labels': labels[rel_label_idxs],
            })
        ...
        return all_boxes, all_scores, all_labels, all_relational_data
    ...
    def forward(self,
        ...
        box_features = self.box_roi_pool(features, proposals, image_shapes)  # [1024, 256, 7, 7]

        if self.object_relation_module_fc1 is not None or self.object_relation_module_fc2 is not None:
            features_per_img = [proposal.shape[0] for proposal in proposals]
            position_matrices = ObjectRelationModuleWithAttention.extract_position_matrix(proposals)
            position_embeddings = ObjectRelationModuleWithAttention.extract_position_embedding(position_matrices)

            if type(self.box_head).__name__ == 'FastRCNNConvFCHead':
                box_features = self.box_head(box_features,
                                             fc1_processor=lambda x: self.object_relation_module_fc1(
                                                 x.split(features_per_img, dim=0), position_embeddings))
            else:
                box_features = self.box_head(box_features,
                                             fc1_processor=lambda x: self.object_relation_module_fc1(
                                                 x.split(features_per_img, dim=0), position_embeddings),
                                             fc2_processor=lambda x: self.object_relation_module_fc2(
                                                 x.split(features_per_img, dim=0), position_embeddings))  # [1024, 1024]
        else:
            box_features = self.box_head(box_features)  # [1024, 1024]

        class_logits, box_regression = self.box_predictor(box_features)  # [1024, 6 (C)] , [1024, 24 (C * 4)]
        ...
        boxes, scores, labels, relational_data = self.postprocess_detections(class_logits,
                                                                             box_regression, proposals,
                                                                             image_shapes,
                                                                             relational_labels=[1, 2,
                                                                                                4])
            ...
            result.append(
                {
                    ...
                    "labels": labels[i],
                    "scores": scores[i],
                    "relational_data": relational_data[i]
                }
            )
        ...
        if self.has_mask():
            ...
            if self.mask_roi_pool is not None:
                mask_features = self.mask_roi_pool(features, mask_proposals, image_shapes)  # [B * 100, 256, 14, 14]
                mask_features = self.mask_head(mask_features)  # [B * 100, 256, 14, 14]
                mask_logits = self.mask_predictor(mask_features)  # [B * 100, 3 (C), 28, 28]
            else:
            ...
            if targets is not None:
                ...
                rcnn_loss_mask = maskrcnn_loss(
                    ...
                    gt_masks, gt_labels, pos_matched_idxs, included_gt_labels=[3, 4])
                loss_mask = {
                    "loss_mask": rcnn_loss_mask
                }
            else:
                labels = [r["labels"] for r in result]
                masks_probs = maskrcnn_inference(mask_logits, labels, excluded_labels=[1, 2, 5])
