...
def _validate_item(item) -> bool:
    ...
    for other_row in other_rows:
        source_bb = [
            item_row['x_0'], item_row['y_0'], item_row['x1'], item_row['y_1']
        ]
        target_bb = [
            other_row['x_0'], other_row['y_0'], other_row['x1'],
            other_row['y_1']
        ]
...
def save_all_dataset():
    dataset = search_files(
        '.jpg',
        comics_crops_dir,
        filename_condition=lambda filename: 'faces' in filename,
        limit=limit_search_files,
        enable_tqdm=True)
    all_item_paths_df = pd.DataFrame(dataset, columns=['img_path'])
    all_item_paths_df.to_csv(dataset_path, encoding='utf-8', index=False)
...
def load_paths(min_comics_series: int = None,
    ...
    if min_comics_series is not None and max_comics_series is not None:
        ...
        def filter_func(x):
            item_type_folder = 'faces' if item_type == 'face' else 'bodies'
            if item_type_folder not in x:
                return False
...
def create_filtered_csv():
    ...
    all_paths = [
        x for x in tqdm(all_paths, f'filtering {item_type} dataset...')
        if _validate_item(x)
    ]
    print('filtered dataset length: ', len(all_paths))
    print('invalidation counts...')
    print(invalidation_row_not_found, invalidation_score_count,
          invalidation_width_count, invalidation_height_count,
          invalidation_intersection_count)
    all_item_paths_df = pd.DataFrame(all_paths, columns=['img_path'])
    all_item_paths_df.to_csv(
        f'filtered_{item_type}_{min_comics_series}-{max_comics_series}.csv',
        encoding='utf-8',
        index=False)
...
def merge_filtered_csvs():
    ...
    csv_files = list(filter(lambda x: f'filtered_{item_type}' in x, csv_files))
    breakpoint()
    df_concat = pd.concat([pd.read_csv(f) for f in csv_files],
                          ignore_index=True)
    df_concat.to_csv(f'filtered_all_{item_type}.csv',
                     encoding='utf-8',
                     index=False)


def clean_filtered_csv():
    filtered_all_csv_path = '/scratch/users/gsoykan20/projects/amazing-mysteries-of-gutter-demystified/data/ssl/filtered_all_face.csv'
    df_concat = pd.read_csv(filtered_all_csv_path)
    df_concat = df_concat[~df_concat['img_path'].str.contains('bodies')]
    df_concat.to_csv(f'filtered_all_{item_type}.csv',
                     encoding='utf-8',
                     index=False)
